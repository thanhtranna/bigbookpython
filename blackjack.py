...
def get_move(player_hand, money):
    ...
    while True:  # Keep looping until the player enters a correct move.
        # Determine what moves the playter can make:
        moves = ['(H)it', '(S)tand']
...
def get_deck():
    ...
    for suit in (HEARTS, DIAMONDS, SPADES, CLUBS):
        ...
        for rank in ('J', 'Q', 'K', 'A'):
            deck.append((rank, suit))  # add the dace and ace cards.
...
def get_hand_value(cards):
    ...
    for card in cards:
        rank = card[0]  # card in a tuple like (rank, suit
        if rank == 'A':
            number_of_aces += 1
